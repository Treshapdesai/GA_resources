...
### 1. Copy the git URI 
from `https://git.generalassemb.ly/DSI-SG-21/materials`
...
```
~/original_materials $ git clone https://git.generalassemb.ly/DSI-SG-21/materials.git .
```
...
`origin  https://git.generalassemb.ly/DSI-SG-21/materials.git (fetch)`<br>
`origin  https://git.generalassemb.ly/DSI-SG-21/materials.git (push)`
...
In this example I will name the repo `my_repo_name`, then click "create repository"

<img src="assets\create_repo.png" width="600"/>

In the next page, you will find your git uri that looks like 
<br>`https://git.generalassemb.ly/2wnh/my_materials.git`<br>
...
```
~/my_materials $ git clone https://git.generalassemb.ly/2wnh/my_materials.git .
```
**Please don't copy mine but use your own URI instead**
...
- `origin` is the shorthand for `https://git.generalassemb.ly/2wnh/my_materials.git` URI
...
Go to `settings` tab on top, then on the left, go to `Collaborators` and add Conor and us to the list
- conorsmyth
- nemoyjh
- 2wnh

Once you have done that, fill up this form: https://forms.gle/QzZ6DNX12gVaUVkH8
